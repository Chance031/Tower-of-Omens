...
std::vector<InventoryItem> BuildInventoryItems(const Player& player)
{
    ...
    {
        items.push_back({"회복약", "HP를 35 회복하는 소모품이다.", player.potionCount});
    }

    if (player.etherCount > 0)
    {
        items.push_back({"마나약", "MP를 20 회복하는 소모품이다.", player.etherCount});
    }

    if (!player.bagWeaponName.empty())
    {
        items.push_back({player.bagWeaponName, "장착 시 무기 보너스가 적용된다.", 1});
    }

    if (!player.bagArmorName.empty())
    {
        items.push_back({player.bagArmorName, "장착 시 방어구 보너스가 적용된다.", 1});
    }
...
std::string ComposeStatBody(const Player& player)
{
    ...
    body << "HP +10 / MP +8 / ATK +2 / DEF +2 중 하나를 선택한다.\n";
    return body.str();
}
...
std::string ComposeInventoryBody(const Player& player, const InventoryItem* item)
{
    ...
    if (item == nullptr)
    {
        ...
        body << "회복약, 마나약, 예비 장비를 획득하면 이곳에 표시된다.\n";
        return body.str();
    }
